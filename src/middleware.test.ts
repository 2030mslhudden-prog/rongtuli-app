...
import { getProductSubmissionStatus } from './lib/auth';
...
test('redirects non-admin authors from dashboard routes', async () => {
  ...
  const request = {
    ...
    cookies: { get: (name: string) => (name === 'rongtuli_auth_token' ? { value: authorToken } : undefined) },
...
test('allows admin sessions to access dashboard routes', async () => {
  ...
  const request = {
    nextUrl: new URL('http://localhost/dashboard/assets'),
    cookies: { get: (name: string) => (name === 'rongtuli_auth_token' ? { value: adminToken } : undefined) },
    url: 'http://localhost/dashboard/assets',
  } as any;
...
test('sends standard uploads to review queue while keeping admin uploads live', () => {
  assert.equal(getProductSubmissionStatus('AUTHOR'), 'PENDING_REVIEW');
  assert.equal(getProductSubmissionStatus('COMMERCIAL_MEMBER'), 'PENDING_REVIEW');
  assert.equal(getProductSubmissionStatus('ADMIN'), 'ACTIVE');
});
